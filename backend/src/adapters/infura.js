...
class InfuraAdapter {
  constructor() {
    // Default to mainnet public endpoint if no key provided
    this.endpoint =
  ...
  async testCall() {
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(this.endpoint, options, (res) => {
        res.on('data', () => {});
        res.on('end', () => {
          // Infura might treat invalid key as 401, but we still measured latency.
          // However for validity we should check 200.
          const latency = Date.now() - startTime;
          if (res.statusCode === 200 || res.statusCode === 401) {
            // Allow 401 for demo purposes if key is missing, to show *some* latency?
            // PRD says "similar to alchemy". Let's stick to 200 only for "success".
...
module.exports = new InfuraAdapter();
